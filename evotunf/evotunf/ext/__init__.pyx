...
cdef extern from "evotunf_ext.h":
    ...
    ctypedef packed struct GaussParams:
        float mu
        float sigma
    void predict_cpu_impl(unsigned *fsets_lens, GaussParams *gauss_params, unsigned char *rules, unsigned rules_len, unsigned n,
                          GaussParams *xxs, unsigned *ys, unsigned N)
    void tune_lfs_cpu_impl(unsigned *fsets_lens, unsigned rules_len, unsigned n, GaussParams *xxs, unsigned *ys, unsigned N,
...
def tune_lfs_cpu(
    ...
    *, rules_len=10, mu=100, lamda=500, iterations=500):
    ...
    tune_lfs_cpu_impl(&fsets_lens[0], rules_len, n, &xx_train[0, 0], &y_train[0], N, mu, lamda, iterations, &fsets_table[0], &rules[0, 0])
...
def predict_cpu(
    ...
    N, n, rules_len = xxs.shape[0], rules.shape[1], rules.shape[0]
    cdef np.ndarray[unsigned, ndim=1, mode='c'] ys = np.zeros(N, dtype=np.uint32)
    predict_cpu_impl(&fsets_lens[0], &fsets_table[0], &rules[0, 0], rules_len, n, &xxs[0, 0], &ys[0], N)
    return ys
