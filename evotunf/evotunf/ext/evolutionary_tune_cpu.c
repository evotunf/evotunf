														...
														Chromosome *population, unsigned population_power)
{
  ...
  for (k = 0; k < population_power; ++k) {
    GaussParams *fsets = population[k].fsets = malloc(sizeof(GaussParams[fsets_total_len]));
    signed char *rules = population[k].rules = malloc(ALIGN_UP(sizeof(signed char[rules_len*(n+1)]), 8));
															 ...
															 const GaussParams *uxx)
{
  ...
  for (k = 0; k < rules_len; ++k) {
    ...
    float uy_center = fsets[fset_offsets[n] + (b-1)].mu;
    float cross = 1.f;

    for (j = 0; j < rules_len; ++j) {
      signed char b = rules[j * (n+1) + n];
      GaussParams ub = fsets[fset_offsets[n] + (b-1)];
      ...
      for (i = 0; i < n; ++i) {
				...
				GaussParams ua = fsets[fset_offsets[i] + (a-1)];
...

  {
    ...
    for (j = 0; j < fset_lens[n]; ++j) {
      GaussParams ub = fsets[fset_offsets[n] + j];
...
{
  memcpy(chd->fsets, chs->fsets, sizeof(GaussParams[fsets_total_len]));
																 ...
																 Chromosome *chx, Chromosome *chy)
{
  ...
  if (pos < fsets_total_len) {
    for (i = 0; i < pos; ++i) {
      SWAP(cha->fsets[i], chb->fsets[i]);
      /* printf("%d %p %p\n", i, cha->fsets + i, chb->fsets + i); */
      /* __auto_type tmp = cha->fsets[i]; */
      /* chb->fsets[i] = tmp; */
    }
  } else {
    memcpy(chx->fsets, cha->fsets, sizeof(GaussParams[fsets_total_len]));
    memcpy(chy->fsets, chb->fsets, sizeof(GaussParams[fsets_total_len]));
    pos -= fsets_total_len;
...
static void perform_mutation(const unsigned *fset_lens, unsigned fsets_total_len, unsigned n,
														 unsigned rules_len, Chromosome *ch, float pm)
{
  /* { */
  /*   unsigned i; */
											 ...
											 GaussParams *fsets, signed char *rules)
{
  ...
  Chromosome* population = malloc(sizeof(Chromosome[population_power]));
  Chromosome* new_population = malloc(sizeof(Chromosome[population_power]));
  float* scores = malloc(sizeof(float[population_power]));
  unsigned* indices = malloc(sizeof(unsigned[population_power]));
  ...
  float pc = 0.9f, pm = 1.0 / (rules_len * n);
...
#pragma omp parallel default(none)																			\
  ...
  private(it, k)																												\
  firstprivate(fsets, rules, pc, pm, f, fs, fpp,												\
...
#pragma omp flush(last_best_score,last_best_update_it)
				...
				memcpy(fsets, population[max_index].fsets, sizeof(GaussParams[fsets_total_len]));
...
#pragma omp for schedule(dynamic, 4)
    for (k = 0; k < population_power; ++k) {
      perform_mutation(fset_lens, fsets_total_len, n, rules_len, new_population + k, pm);
    }
